"""
テスト用のモックユーザー・レビューデータをDBに投入するスクリプト。

実行方法:
    docker compose exec backend python -m app.scripts.seed_data

何度実行しても、その都度ユーザー・レビューが追加登録される(重複チェックはしていない)。
ログイン画面・履歴機能の動作確認用としてだけ使う想定。
"""
import random
...
# ログイン確認用のテストアカウント(メールアドレス・パスワードは控えておく)
MOCK_USERS = [
...
def seed(review_count: int = 25) -> None:
    db = SessionLocal()
    try:
        # 1. テストユーザーを作成
        users = []
        for mock_user in MOCK_USERS:
            user = User(
                ...
                hashed_password=hash_password(mock_user["password"]),
            )
            db.add(user)
            users.append(user)
        db.commit()
        ...
        print(f"{len(users)}件のユーザーと{review_count}件のレビューを投入しました")
        print("ログイン確認用アカウント:")
        for mock_user in MOCK_USERS:
            print(f"  email: {mock_user['email']} / password: {mock_user['password']}")
    finally:
        db.close()
